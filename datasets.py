...
def extract_vgg16_features(x):
 ...
 # if flatten:
    ...
    feature_model = Model(inputs=model.input, outputs=top_model(model.output))
...
def extract_myvgg16_features(x):
    from keras.preprocessing.image import img_to_array, array_to_img
    from keras.applications.vgg16 import preprocess_input, VGG16
    from keras.models import Model
    from keras.models import Sequential
...
def load_stl(data_path='./media/HDD_3TB/dcharua/DEC-keras/data/stl'):
    import os
...
def load_egocentric(data_path='./data/egocentric'):
    import os

    with h5py.File('data/egocentric/data4torch.h5', 'r') as hf:
    	y = hf['labels'][:]
...
def load_egofull(data_path='./data/egofull'):
    import os
...
def load_data(dataset_name):
    if dataset_name == 'mnist':
        ...
        return load_stl()
    elif dataset_name == 'egocentric':
        return load_egocentric()
    elif dataset_name =='egofull':
        return load_egofull()
    else:
